...
public class TradeServletAction {
	...
	void doAccount(ServletContext ctx, HttpServletRequest req, HttpServletResponse resp, String userID, String results)
		...
		try {
			...
			KYCBean KYC = tAction.getKYC(userID);
			// Collection<?> orderDataBeans = (TradeConfig.getLongRun() ? new
			// ArrayList<Object>() : (Collection<?>) tAction.getOrders(userID));
			Collection<?> orderDataBeans = ((Collection<?>) tAction.getOrders(userID));
			...
			req.setAttribute("KYC", KYC == null ? null : KYC.getKYCName());
	...
	void doReport(ServletContext ctx, HttpServletRequest req, HttpServletResponse resp, String userID, String symbols)
			throws ServletException, IOException {
		try {
			Date date = new Date();
			boolean yearly = (req.getParameter("timing") != null
					&& req.getParameter("timing").toString().equals("yearly"));
			Collection<?> orderDataBeans = tAction.getOrdersByDate(userID, date, yearly);
			req.setAttribute("orderDataBeans", orderDataBeans);

			DateFormat dateFormat = new SimpleDateFormat("yyyy-mm-dd");
			String strDate = dateFormat.format(date);
			System.out.println("strDate " + strDate);
			req.setAttribute("current_date", strDate);
			if (yearly) {
				req.setAttribute("checked_timing", yearly);
			}
			requestDispatch(ctx, req, resp, userID, TradeConfig.getPage(TradeConfig.REPORT_PAGE));
		} catch (Exception e) {
			// log the exception with error page
			throw new ServletException("TradeServletAction.doAccount(...)" + " exception user =" + userID, e);
		}
	...
	void doDownloadReport(ServletContext ctx, HttpServletRequest req, HttpServletResponse resp, String userID,
			String symbols, String date) throws ServletException, IOException {
		try {

			DateFormat dateFormat = new SimpleDateFormat("yyyy-mm-dd");
			Date newDate = dateFormat.parse(date);
			Collection<?> orderDataBeans = tAction.getOrdersByDate(userID, newDate, false);
			...
			Iterator<OrderDataBean> itr = (Iterator<OrderDataBean>) orderDataBeans.iterator();
			while (itr.hasNext()) {
				sb.append(itr.next().toCSV());
				// reportWriter.write(itr.next().toCSV());
			}
	...
	public void doDownloadKYC(ServletContext ctx, HttpServletRequest req, HttpServletResponse resp, String userID) {
		try {
			KYCBean KYC = tAction.getKYC(userID);
			String filename = KYC.getKYCName();
	...
	void doFileUpload(Part kyc, String uid, String fileName) {
		System.out.println("do file upload success! with kyc: " + kyc.toString() + "\n and uid: " + uid);
		tAction.fileUpload(uid, kyc, fileName);
	}
